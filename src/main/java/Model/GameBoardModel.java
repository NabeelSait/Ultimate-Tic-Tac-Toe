...
public class GameBoardModel implements GameModel {
    ...
    {
      GamePanelModel[] panels = new GamePanelModel[9];
      for (int i = 0; i < 9; i++)
      {
         panels[i] = new GamePanelModel();
      }
      _bus = e;
      _panelsfilled = 0;
      _player1 = p1;
    ...
    {
      GamePanelModel panel = panels[m.getBoard()];
      if (!(panel.isClosed))
      {
    ...
    {
      if (panels[m.getPosition()].isClosed)
      {
         return true;
      }
      return false;
